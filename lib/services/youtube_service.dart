...
class YoutubeService {
  ...
  // --- FETCH SONGS FOR A SPECIFIC PLAYLIST (PAGINATED) ---
  Future<FetchResults> fetchPlaylistSongs(String playlistId, {String? pageToken}) async {
    try {
      List<SongModel> fetchedSongs = [];
      String urlString = 'https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=$playlistId&maxResults=50';
      
      // If we are scrolling down, attach the token for the next page!
      if (pageToken != null) {
      ...
      var response = await _authenticatedRequest(
        (token) => http.get(url, headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'}),
      );

      if (response != null && response.statusCode == 200) {
        final data = json.decode(response.body);
        final List items = data['items'] ?? [];
        final String? nextToken = data['nextPageToken']; // Grab the token for the next batch
        ...
        for (var item in items) {
          ...
          final coverArt = thumbnails?['maxres']?['url'] ?? thumbnails?['high']?['url'] ?? thumbnails?['default']?['url'] ?? '';
          String title = snippet['title'] ?? 'Unknown Title';
          String cleanArtist = (snippet['videoOwnerChannelTitle'] ?? 'Unknown Artist').replaceAll(' - Topic', '').replaceAll('VEVO', '').trim();

          fetchedSongs.add(SongModel(id: videoId, title: title, artist: cleanArtist, coverArtUrl: coverArt));
        }
        
        // Return BOTH the songs and the token to fetch more later
        return FetchResults(songs: fetchedSongs, nextPageToken: nextToken);
      }
      return FetchResults(songs: []);
    } catch (e) {
      print('BOP EXCEPTION: Failed to fetch playlist songs: $e');
      return FetchResults(songs: []);
    }
  }
